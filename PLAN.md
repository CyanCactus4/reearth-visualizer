...
| 6 История / undo | 🟡 | Mongo + REST + **`CollabApplyHistoryPanel`**; журнал **`collabApplyAudit`** с **`layerId` / `layerIds`** для NLS-`apply`; **серверный** undo/redo (`POST /api/collab/undo|redo`) для **`update_widget`**, **`move_story_block`**, **`move_story_page`**, **`update_story_page`**, **`update_property_value`**, **`update_style`** и **`update_nls_layer`** (запись в стек только если обратимый снимок: нет undo при первом задании **index** с пустого, или при смене **config**, если до этого **config** не было); **нет** undo add/remove виджета и **нет** админского restore ревизии. |
