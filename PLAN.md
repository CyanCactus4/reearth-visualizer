...
| 6 История / undo | 🟡 | Mongo + REST + **`CollabApplyHistoryPanel`**; **серверный** undo/redo (`POST /api/collab/undo|redo`) для **`update_widget`**, **`move_story_block`**, **`update_property_value`** и **`update_style`**; **нет** undo add/remove виджета и **нет** админского restore ревизии. |
