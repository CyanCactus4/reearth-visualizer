...
| 2 MVP-синхронизация одной сущности | ✅ | `apply`: виджеты + story **blocks** + story **pages** (`create`/`remove`/`move` для обоих); `applied` + **`sceneRev`**; Vitest `applyMessages`; фронт: **`useStoryPageMutations(sceneId)`** (для `moveStoryPage` нужен `sceneId`), **`useStoryBlockMutations(sceneId)`**, **`useWidgetMutations`**. |
| 3 OT/CRDT | 🟡 | **`baseSceneRev`** + `stale_state`; storytelling blocks + **pages** (`create`/`remove`/`move`); **LWW `entityClocks`** (+ Redis при `REEARTH_COLLAB_REDIS_URL`). **→ дальше:** CRDT/merge по JSON property, **`update_story_page`**, дубликат страницы, слои/стили/сцена. |
