...
| 6 История / undo | 🟡 | Mongo + REST + **`CollabApplyHistoryPanel`**; журнал **`collabApplyAudit`** с **`layerId` / `layerIds`** для NLS-`apply`, опциональным **`userName`**, а также строками **`collab_undo` / `collab_redo`** и полем **`opKind`** (исходный `kind` из undo-стека); **GET apply-audit** поддерживает **`sceneId`** (только сцена проекта) — панель истории в редакторе запрашивает события **текущей** сцены; **серверный** undo/redo (`POST /api/collab/undo|redo`) для **`update_widget`**, **`move_story_block`**, **`move_story_page`**, **`update_story_page`**, **`update_property_value`**, **`update_style`**, **`update_nls_layer`** и **`update_nls_layers`** (для одиночного и батча — запись в стек только если **каждая** строка обратима теми же правилами: нет undo при первом **index** с пустого и т.д.; inverse батча в порядке, обратном к forward); **нет** undo add/remove виджета и **нет** админского restore ревизии. |
