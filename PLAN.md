...
| 3 OT/CRDT | 🟡 | **`baseSceneRev`** + `stale_state`; storytelling blocks + **pages** (в т.ч. duplicate); **LWW `entityClocks`** (+ Redis при `REEARTH_COLLAB_REDIS_URL`); NLS-слои, **infobox / photo overlay** на NLS (в т.ч. sketch) и **стили слоя (`Style`)** через `apply`. **→ дальше:** CRDT/merge по JSON property; **`sceneId`** в редакторе доходит до plugin blocks (story/infobox) → `PropertyItem`; остаётся дожать редкие обходы без `sceneId` и прочие NLS-подоперации (custom property schema, GeoJSON features, …). |
