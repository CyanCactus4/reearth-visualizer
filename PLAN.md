...
## Формальное закрытие варианта (MVP)

**Дата:** 2026-04-12. По критериям [TASK.md](TASK.md) (FR‑1–6, brownfield MVP) и фазам **0–9** ниже работы по real-time collaboration в этом репозитории **считаются формально завершёнными** для сдачи варианта. Фаза **10** остаётся **🟡** как живой чеклист прод‑харденинга и эксплуатации, а не как блокер зачёта. Опциональный бэклог (полный JSON‑CRDT, расширение NLS, CI fuzz) — см. строку «→ опционально» у фазы 3.

---

## Статус фаз (живой ориентир)

После формального закрытия MVP **приоритет доработок** — фаза **10** и опциональный бэклог из фазы 3. Детальный контракт фазы 0: [docs/design-doc/20260411_001_collaboration_protocol_mvp.md](docs/design-doc/20260411_001_collaboration_protocol_mvp.md).
...
| 2 MVP-синхронизация одной сущности | ✅ | `apply`: виджеты + story **blocks** + story **pages**; **NLS-слои** + **NLS infobox** (`create_nls_infobox` / `remove_nls_infobox` + `add_nls_infobox_block` / `move_nls_infobox_block` / `remove_nls_infobox_block`); **NLS photo overlay (sketch)** (`create_nls_photo_overlay` / `remove_nls_photo_overlay`); **стили слоя (`Style`)**; **`update_property_value`** (значения полей property через interactors); `applied` + **`sceneRev`**; Vitest `applyMessages`; фронт: **`useStoryPageMutations(sceneId)`**, **`useStoryBlockMutations(sceneId)`**, **`useInfoboxMutations(sceneId)`**, **`useInfoboxBlockMutations(sceneId)`**, **`usePhotoOverlayMutations(sceneId)`**, **`useWidgetMutations`**, **`useNLSLayerMutations(sceneId)`**, **`useLayerStyleMutations(sceneId)`**, **`usePropertyMutations(sceneId)`** там, где в инспектор передан **`sceneId`**. |
| 3 OT/CRDT | ✅ | **`baseSceneRev`** + `stale_state`; **виджеты:** **`entityClocks`** (+ Redis); **Property:** **CRDT LWW‑Register + HLC** (`fieldHlc` / `propertyFieldHlc`), legacy **`fieldClock`**, **`merge_property_json`** (leaf merge-patch + **`docClock`** CAS), **`add_property_item` / `remove_property_item` / `move_property_item`**, общий **`propertyCollabApplyMu`** с `merge_property_json`; storytelling + NLS + infobox/photo overlay + **стили** через `apply`. **→ опционально (не ТЗ варианта):** документный CRDT (Yjs/Automerge) для произвольного вложенного JSON плагинов; расширение NLS (custom schema, GeoJSON features); CI нагрузка. |
...
| 5 Presence | ✅ | Курсоры (в т.ч. **`title` = полный userId**), **монограмма или `photoURL`** (из WS `presence` + Accounts metadata) + короткая подпись (TASK.md FR‑3), typing, полоса presence. |
| 6 История / undo | ✅ | Mongo + REST + **`CollabApplyHistoryPanel`**; журнал **`collabApplyAudit`** с **`layerId` / `layerIds`** для NLS-`apply`, опциональным **`userName`**, а также строками **`collab_undo` / `collab_redo`** и полем **`opKind`** (исходный `kind` из undo-стека); **GET apply-audit** поддерживает **`sceneId`** (только сцена проекта) — панель истории в редакторе запрашивает события **текущей** сцены; **серверный** undo/redo (`POST /api/collab/undo|redo`) для **`update_widget`**, **`add_widget`**, **`remove_widget`**, **`move_story_block`**, **`move_story_page`**, **`update_story_page`**, **`update_property_value`**, **`merge_property_json`** (undo → обратный merge-patch по изменённым листьям), **`add_property_item`** (undo → **`remove_property_item`**), **`move_property_item`** (undo → обратный **`move_property_item`** с прежним индексом), **`remove_property_item`** (undo → **`add_property_item`** со снимком представительного поля и индекса; redo forward — новый **`itemId`**), **`update_style`**, **`add_style`**, **`remove_style`**, **`update_nls_layer`**, **`update_nls_layers`** и NLS/infobox/geojson/custom-property (см. `undo_exec.go`); **`ExecuteCollabUndoJSON`** принимает **`hub`** для mutex/часов property-merge при HTTP undo/redo. **ограничение redo:** после undo **`remove_widget`** / **`remove_style`** forward хранит старый **id**. **Админский** restore: **`POST /api/collab/admin/restore-scene`** + Mongo **`collabSceneSnapshots`** (throttle ~45 с), **`ImportSceneData`** по ближайшему `sceneRev <= targetSceneRev`; без Mongo — **501**. **Redo:** патч **`forwardJson`** после undo remove widget/style / remove_property_item. |
| 7 Уведомления и чат | ✅ | Чат + Mongo + toasts по `applied` (в т.ч. **`collab_undo` / `collab_redo`** с **`opKind`** в payload для читаемого toast); **`@mentions`**: парсинг, Mongo/WS, подсветка; **WS `notify`** `chat_mention`; опционально **webhook** `REEARTH_COLLAB_MENTION_WEBHOOK_URL`. |
...
| 9 Фронт Apollo/offline | ✅ | Provider + **`localforage`** offline queue (`offlineQueue.ts`); сброс очереди при **открытии WS**, **`online`** и **`visibilitychange` → visible** при открытом сокете; reconcile сцены из редактора при lock-conflict. |
