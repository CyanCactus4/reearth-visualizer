...
| `apply` | Server-authoritative mutation; `d.kind` ∈ `update_widget`, `add_widget`, `remove_widget`, `move_story_block`, `create_story_block`, `remove_story_block`, `create_story_page`, `remove_story_page`, `move_story_page` (scene writes via interactors). |
...
| **Storytelling** | `apply` kinds: blocks — **`move_story_block`**, **`create_story_block`**, **`remove_story_block`**; pages — **`create_story_page`**, **`remove_story_page`**, **`move_story_page`** (same auth/audit/`applied`/`sceneRev` path as widgets). |
