...
| `apply` | Server-authoritative mutation; `d.kind` ∈ `update_widget`, `add_widget`, `remove_widget`, `move_story_block`, `create_story_block`, `remove_story_block`, `create_story_page`, `remove_story_page`, `move_story_page`, `update_story_page`, `duplicate_story_page`, `add_nls_layer_simple`, `remove_nls_layer`, `update_nls_layer`, `update_nls_layers`, **`create_nls_infobox`**, **`remove_nls_infobox`**, **`add_nls_infobox_block`**, **`move_nls_infobox_block`**, **`remove_nls_infobox_block`**, **`create_nls_photo_overlay`**, **`remove_nls_photo_overlay`**, `add_style`, `update_style`, `remove_style`, **`update_property_value`** (single field/value on a scene-linked **Property** via `Property.UpdateValue`; bumps **scene** revision) (scene writes via interactors). |
...
| **NLS layers** | **`add_nls_layer_simple`**, **`remove_nls_layer`**, **`update_nls_layer`**; batch reorder / multi-field — **`update_nls_layers`**; **NLS infobox** — **`create_nls_infobox`** / **`remove_nls_infobox`**; **infobox plugin blocks** — **`add_nls_infobox_block`**, **`move_nls_infobox_block`**, **`remove_nls_infobox_block`**; **sketch photo overlay** — **`create_nls_photo_overlay`**, **`remove_nls_photo_overlay`** (`NLSLayer.Create/RemoveNLSPhotoOverlay`; optional **layer** collab-lock, same `applied` / `sceneRev` / audit path). |
