...
| `apply` | Server-authoritative mutation; `d.kind` ∈ `update_widget`, `add_widget`, `remove_widget`, `move_story_block`, `create_story_block`, `remove_story_block` (scene writes via interactors). |
...
| **Storytelling** | `apply` kinds **`move_story_block`** → `MoveBlock`, **`create_story_block`** → `CreateBlock`, **`remove_story_block`** → `RemoveBlock` (same auth/audit/`applied`/`sceneRev` path as widgets). |
