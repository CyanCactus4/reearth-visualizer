...
- Совместное редактирование (MVP): **`GET /api/collab/ws?projectId=...`** — WebSocket после той же приватной авторизации, что и GraphQL; опционально **`REEARTH_COLLAB_REDIS_URL`** для pub/sub между инстансами (см. `server/.env.example`). Сообщения JSON v1: **`ping`**, **`relay`** (рассылка), **`apply`** с `d.kind=update_widget` (сервер вызывает `Scene.UpdateWidget` при праве на запись сцены). Контекст запроса для длительного соединения: **`context.WithoutCancel`**. Клиент: `web/src/services/collab/` (`buildCollabWsUrl`, `CollabClient`, **`CollabProvider`** / **`useCollab`** — редактор обёрнут в `CollabProvider` в `Editor/index.tsx`).
