...
- Совместное редактирование (MVP): **`GET /api/collab/ws?projectId=...`** — WebSocket после той же приватной авторизации, что и GraphQL; опционально **`REEARTH_COLLAB_REDIS_URL`** для pub/sub между инстансами и **для блокировок между инстансами** (Lua + `SET NX` в Redis; без URL — in-memory на процесс). Сообщения JSON v1: **`ping`**, **`relay`** (рассылка), **`apply`** с `d.kind=update_widget` (сервер вызывает `Scene.UpdateWidget` при праве на запись сцены), серверные **`presence`** (`d.event` = `join`/`leave`, `d.userId`), **`chat`** (`d.text`) → рассылка **`chat`** с лимитом длины (**`REEARTH_COLLAB_CHAT_MAX_RUNES`**) и **rate limit** на пользователя/проект (**`REEARTH_COLLAB_CHAT_MIN_INTERVAL_MS`**), **`lock`** (`d.action` = `acquire`/`release`/`heartbeat`, `d.resource` = `layer`|`widget`, `d.id`) → **`lock_changed`** / **`lock_denied`**; TTL блокировки **`REEARTH_COLLAB_LOCK_TTL_SECONDS`** (по умолчанию 300 с). Контекст запроса для длительного соединения: **`context.WithoutCancel`**. Клиент: `web/src/services/collab/` (`buildCollabWsUrl`, `CollabClient`, **`CollabProvider`** / **`useCollab`**, очередь offline в IndexedDB через **`offlineQueue`**, редактор обёрнут в `CollabProvider` в `Editor/index.tsx`, полоска присутствия **`CollabPresenceBar`**).
