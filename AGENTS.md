...
- Совместное редактирование (MVP): **`GET /api/collab/ws?projectId=...`** — WebSocket после той же приватной авторизации, что и GraphQL; **`GET /api/collab/chat?projectId=&limit=`** — история чата в Mongo (коллекция по умолчанию `collabChatMessages`, имя через **`REEARTH_COLLAB_CHAT_COLLECTION`**), индекс **`{ projectId: 1, ts: -1 }`** создаётся при старте. Опционально **`REEARTH_COLLAB_REDIS_URL`** для pub/sub между инстансами и **для блокировок между инстансами** (Lua + `SET NX` в Redis; без URL — in-memory на процесс). Сообщения JSON v1: **`ping`**, **`relay`**, **`apply`** (`d.kind=update_widget` → `Scene.UpdateWidget` при праве на запись; после успеха — **`applied`** с `userId`, `kind`, `widgetId`, `sceneId`), **`presence`** (`join`/`leave`, `userId`), **`chat`** (персистентность + rate limit), **`cursor`**, **`activity`**, **`lock`** → **`lock_changed`** / **`lock_denied`**; TTL блокировок **`REEARTH_COLLAB_LOCK_TTL_SECONDS`**. Контекст: **`context.WithoutCancel`**. Клиент: `web/src/services/collab/` + **`CollabViewportCapture`** / **`CollabRemoteCursors`**, **`CollabPresenceBar`**, **`CollabChatPanel`**, **`CollabLockGate`** / **`CollabLockReadOnly`** / **`CollabLockLeaseOnly`**: слой (lease на уровне вкладки Map при выборе), инспектор слоя без дублирующего lease, виджет, **контейнер виджетов (`widget_area`)**, **сцена (`scene`)**; при **`lock_denied`** — toast + модалка «конфликт» (MVP без merge); при **`applied`** от другого пользователя — info-toast (throttle ~4s на пару user+target); карта **`resourceLocks`** в контексте, offline **`offlineQueue`**, optimistic чат при известном **`localUserId`**, в редакторе передаётся **`localUserId`** из **`useMe`**.
