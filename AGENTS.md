...
- Совместное редактирование (MVP): **`GET /api/collab/ws?projectId=...`** — WebSocket после той же приватной авторизации, что и GraphQL; опционально **`REEARTH_COLLAB_REDIS_URL`** для pub/sub между инстансами (см. `server/.env.example`). Сообщения JSON v1: **`ping`**, **`relay`** (рассылка), **`apply`** с `d.kind=update_widget` (сервер вызывает `Scene.UpdateWidget` при праве на запись сцены), серверные **`presence`** (`d.event` = `join`/`leave`, `d.userId`), **`lock`** (`d.action` = `acquire`/`release`/`heartbeat`, `d.resource` = `layer`|`widget`, `d.id`) → **`lock_changed`** / **`lock_denied`**; TTL блокировки бездействия **`REEARTH_COLLAB_LOCK_TTL_SECONDS`** (по умолчанию 300 с). Таблица блокировок **in-memory на процесс** (несколько инстансов — позже через Redis). Контекст запроса для длительного соединения: **`context.WithoutCancel`**. Клиент: `web/src/services/collab/` (`buildCollabWsUrl`, `CollabClient`, **`CollabProvider`** / **`useCollab`** — редактор обёрнут в `CollabProvider` в `Editor/index.tsx`).
...
- В репозитории **нет** готовой интеграции **Redis**, **gorilla/websocket** для доменного collab, OT/CRDT для сцены — это предстоит внедрить согласно плану в [PLAN.md](PLAN.md).
