...
- **POST /api/collab/undo|redo** — тело `{"sceneId":"<id>"}`; стек в Mongo (см. [docs/collab-production-deploy.md](docs/collab-production-deploy.md)); выполнение inverse/forward через **`server/internal/collab/undo_exec.go`** → **`ExecuteCollabUndoJSON(ctx, raw, op, hub)`** — для **`merge_property_json`** и согласованности property на undo/redo передаётся живой **`Hub`** (mutex **`propertyCollabApplyMu`**, doc/field clocks). Клиентские коды ошибок apply с refetch сцены: **`COLLAB_APPLY_FAILURE_SCENE_REFETCH_CODES`** в `web/src/services/collab/collabApplyErrorCodes.ts` (в т.ч. **`stale_property_field`**, **`stale_property_doc`**).
- Совместное редактирование (MVP): **`GET /api/collab/ws?projectId=...`** — WebSocket после той же приватной авторизации, что и GraphQL; **`GET /api/collab/scene-rev/stream?sceneId=…`** — **SSE** (`text/event-stream`): строки `data: <sceneRevMs>` при успешном collab-`apply` для этой сцены; при **`REEARTH_COLLAB_REDIS_URL`** те же обновления **рассылаются между инстансами API** (канал `collab:srev:<sceneId>`), чтобы SSE и **`subscription { collabSceneRevision }`** работали не только в процессе, где пришёл `apply` (см. [docs/collab-production-deploy.md](docs/collab-production-deploy.md)). **`GET /api/collab/apply-audit?projectId=&limit=`** — чтение журнала успешных **`apply`** (JSON `entries`, newest-first; Mongo **`collabApplyAudit`**). **`GET /api/collab/chat?projectId=&limit=`** — история чата в Mongo (коллекция по умолчанию `collabChatMessages`, имя через **`REEARTH_COLLAB_CHAT_COLLECTION`**; в документе опционально **`mentions`** из `@handle`), индекс **`{ projectId: 1, ts: -1 }`**; append-only журнал успешных **`apply`** — коллекция по умолчанию **`collabApplyAudit`** (имя через **`REEARTH_COLLAB_APPLY_AUDIT_COLLECTION`**), индекс **`{ projectId: 1, ts: -1 }`** — при старте. Опционально **`REEARTH_COLLAB_REDIS_URL`** для pub/sub между инстансами и **для блокировок между инстансами** (Lua + `SET NX` в Redis; без URL — in-memory на процесс). Сообщения JSON v1: **`ping`**, **`relay`**, **`apply`** (`d.kind` ∈ `update_widget` / `add_widget` / `remove_widget` / `move_story_block` / `create_story_block` / `remove_story_block` / `create_story_page` / `remove_story_page` / `move_story_page` / `update_story_page` / **`duplicate_story_page`** / **`add_nls_layer_simple`** / **`remove_nls_layer`** / **`update_nls_layer`** / **`update_nls_layers`** / **`add_style`** / **`update_style`** / **`remove_style`** / **`update_property_value`** (HLC **`fieldHlc`** или legacy **`fieldClock`**) / **`merge_property_json`** / **`add_property_item`** / **`remove_property_item`** / **`move_property_item`** → `Scene` / **`StoryTelling`** / **NLS** / **стили слоя (`Style`)** / **поля Property**; опционально **`baseSceneRev`** — при несовпадении с `scene.UpdatedAt` ms → `error` **`stale_state`**; при активном **collab-lock** на виджет другим пользователем — `error` **`object_locked`**; после успеха — **`applied`** с `userId`, `kind`, `sceneRev`, идентификаторами), **`presence`** (`join`/`leave`, `userId`), **`chat`** (персистентность + rate limit; **`notify`** `chat_mention` адресно по `userId`), **`cursor`**, **`activity`**, **`lock`** → **`lock_changed`** / **`lock_denied`**; ресурсы: **`layer`**, **`widget`**, **`scene`**, **`widget_area`**, **`style`** (id стиля сцены). TTL блокировок **`REEARTH_COLLAB_LOCK_TTL_SECONDS`**. Контекст: **`context.WithoutCancel`**. Клиент: `web/src/services/collab/` + **`CollabViewportCapture`** / **`CollabRemoteCursors`**, **`CollabPresenceBar`**, **`CollabChatPanel`**, **`CollabSceneRefetch`** (Apollo GetScene при новом `sceneRev`), **`CollabLockGate`** / **`CollabLockReadOnly`** / **`CollabLockLeaseOnly`**: слой (lease на уровне вкладки Map при выборе), инспектор слоя без дублирующего lease, виджет, **контейнер виджетов (`widget_area`)**, **сцена (`scene`)**; при **`lock_denied`** — toast + модалка «конфликт» (MVP без merge); при **`applied`** от другого пользователя — info-toast (throttle ~4s на пару user+target); карта **`resourceLocks`** в контексте, offline **`offlineQueue`**, optimistic чат при известном **`localUserId`**, в редакторе передаётся **`localUserId`** из **`useMe`**.
- Обработчик gqlgen: `transport.Websocket` в `server/internal/app/graphql.go`; **GET и POST** на `/api/graphql` — WebSocket для **`subscription { collabSceneRevision(sceneId) }`** (см. `server/gql/_shared.graphql`). После правок схемы в офлайне см. `docs/collab-production-deploy.md` (ручной патч `generated.go` / `make gql`).
- Фронт: `web/src/services/gql/provider/index.tsx` — **split link**: HTTP upload + **`graphql-ws`** для подписок.
